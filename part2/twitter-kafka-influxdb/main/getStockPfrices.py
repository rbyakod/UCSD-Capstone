...
org = "ravi-org-1"
bucket = "ravi-org-1-bucket"
token = "yswFR92iYy6HFXnxhUori4n7NXULy7JdPR7VetGBrr7DjCY3Y4vZ3ig5PoLhVnp65LrMuNCeA8QuCYNZh1ackg=="

IC = InfluxClient(token, org, bucket)

'''
get data for TSLA stock
and save
'''
data = yf.download("TSLA", start="2022-01-01", end="2022-08-01")
print(data.to_csv("tsla-stock-prices.csv"))


'''
Write Data for TSLA Stock
'''
TSLA_file = open('tsla-stock-prices.csv')
csvreader = csv.reader(TSLA_file)
header = next(csvreader)
rows = []
for row in csvreader:
    date, open, high, low = row[0], row[1], row[2], row[3]
    line_protocol_string = ''
    line_protocol_string += f'TSLA_{date},'
    line_protocol_string += f'stock=TSLA '
